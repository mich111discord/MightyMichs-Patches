...
val unlockPremiumPatch = bytecodePatch(
    ...
    //    Note: we do NOT restrict the return type, because the target method e() returns void.
    val purchaseCheckFingerprint = Fingerprint(
        strings = listOf("purchase_buy__")
    )

    execute {
        // 2. Retrieve the matched method.
        ...
        // 3. Find the index of the "move-result" instruction that follows getBoolean.
        //    This is where the result of the purchase check is stored.
        val instructions = method.implementation!!.instructions.toList()
        var insertIndex = -1

        for (i in instructions.indices) {
            val instr = instructions[i]
            // Look for a move-result instruction...
            if (instr.opcode.name == "MOVE_RESULT" || instr.opcode.name == "MOVE_RESULT_OBJECT") {
                // ...and check if the previous instruction is a call to getBoolean.
                if (i > 0) {
        ...
        // 4. Insert "const/4 p0, 0x1" right after move-result p0.
